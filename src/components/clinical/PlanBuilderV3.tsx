...
export function PlanBuilderV3({ patient, onSave, initialPlan, customSaveHandler }: PlanBuilderV3Props) {
    ...
    if (isLibraryItem) {
        // Find target block (if dropped on a specific block container or item)
        // Default to MAIN if dropped on background or unknown
        let targetBlock = SESSION_BLOCKS.MAIN;

        if (over) {
            const overId = over.id as string;
            // If over is a container (block key)
            if (Object.values(SESSION_BLOCKS).includes(overId as any) || Object.keys(SESSION_BLOCKS).includes(overId.toUpperCase())) {
                // Check if it matches a block value or key? 
                // Our droppables will be the Block Values e.g. "Calentamiento"
                targetBlock = overId as any;
            } else {
                // If over an item, find that item's block
                const overItem = plan.schedule[activeDay].find(i => i.id === overId);
                if (overItem?.block) targetBlock = overItem.block;
            }
        }

        handleAddExercise(activeData as Exercise, targetBlock);
        return;
    }

    // 2. Reorder within Active Day (and potentially changes Block)
    // Handled by handleDragOver mostly, but confirm final position here
    const activeItem = plan.schedule[activeDay].find(x => x.id === active.id);
    const overItem = plan.schedule[activeDay].find(x => x.id === over.id);

    if (!activeItem) return;

    // If dropped on a Container (Empty Block)
    if (over.data.current?.sortable?.containerId !== activeItem.block && over.data.current?.type === 'container') {
        // Moved to empty container
        // handleDragOver should have handled this visual update? 
        // dnd-kit is tricky. Let's rely on arrayMove in dragEnd ONLY for sorting.
        // Block changes happen in dragOver or here.
    }
};
